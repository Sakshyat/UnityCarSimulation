...
public class ACOCON
{
    ...
    */
    public List<Connection> ACO(int IterationThreshold, int TotalNumAnts, GameObject[] WaypointNodes,
        ...
        for (int i = 0; i < IterationThreshold; i++)
        {
            for (int i2 = 0; i2 < TotalNumAnts; i2++)
            {
                ...
                while (VisitedNodes.Count < WaypointNodes.Length)
                {
                    ...
                    // Calculate the product of the pheromone level and the visibility factor of theproposed path.
    ...
    private void LogConnections(List<Connection> Connections)
    {
        foreach (Connection aConnection in Connections)
        {
            Debug.Log(">" + aConnection.GetFromNode().name + " | ---> " + aConnection.GetToNode().name +
            " = " + aConnection.GetPheromoneLevel());
        }
    }
    // Log Route
    private void LogRoute(GameObject StartNode, int MaxPath, GameObject[] WaypointNodes,
    List<Connection> Connections)
    {
        ...
        while (CurrentNode != null)
        {
            ...
            if (AllFromConnections.Count > 0)
            {
                ...
                CurrentNode = HighestPheromoneConnection.GetToNode();
                Output += "| FROM: " + HighestPheromoneConnection.GetFromNode().name + ", TO: " +
            ...
            {
                CurrentNode = null;
                Output += "MAX PATH (Total Nodes:" + WaypointNodes.Length + ", Nodes in Route: " +
                PathCount + ").\n";
            }
            PathCount++;
        }
